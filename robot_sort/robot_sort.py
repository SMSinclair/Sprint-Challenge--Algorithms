class SortingRobot:
    ...
    def sort(self):
        ...
        while(self.light_is_on()):
            self.set_light_off() #light off, no swaps yet
            while(self.can_move_left()):
                self.move_left()

            while(self.can_move_right()):
                self.swap_item() # pick up item 1
                #print(f"picked up {self._item}")
                self.move_right() # move right
                #print("move right")
                if(self.compare_item()==1): #compare
                    self.set_light_on() # if item 2 is greater swap and turn light on
                    self.swap_item() 
                    #print(f"swapped with {self._item}, light is {self._light}")
               
                self.move_left() # reset for next iteration
                self.swap_item()
